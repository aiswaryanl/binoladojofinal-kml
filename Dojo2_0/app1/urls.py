...
# 
    ...
    # Get the full list of roles and modules for the Matrix UI
    path('permission-matrix/', PermissionMatrixView.as_view(), name='permission-matrix'),
    
    # Get/Update permissions for a specific role
    path('roles/<int:role_id>/permissions/', RolePermissionsDetailView.as_view(), name='role-permissions'),
    
    # Optional: Manage modules (add/edit Tiles or Links)
    path('modules-management/', ModuleManagementView.as_view(), name='modules-management'),

    # Sync Modules from Frontend
    path('sync-modules/', SyncModulesView.as_view(), name='sync-modules'),
